...
function wp360invoice_showInvoice($invoiceID){
    ...
    if ( !$invoicePost || get_post_type($invoiceID) !== 'wp360_invoice' || $invoicePost->post_status !== 'publish' ) {
        _e('Invice not found!', 'wp360-invoice');
    }
    if ($invoicePost && get_post_type($invoiceID) === 'wp360_invoice' && $invoicePost->post_status === 'publish'):
    ...
    ?>
        <div id="wp360-invoice_printinvoice" class="hidden-print" data-id="<?php echo esc_attr($invoiceID);?>"><?php esc_html_e('Print Invoice', 'wp360-invoice')?></div>
                ...
                <table class="invoiceSummary">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo ucfirst(esc_html($invoicetype)); ?></th>
